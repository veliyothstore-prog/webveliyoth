            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
              {data.promotions.map(promo => (
                <div key={promo.id} style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0', textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', textAlign: 'left' }}><label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', fontWeight: 600 }}><input type="checkbox" checked={promo.active} onChange={(e) => updatePromotion(promo.id, { active: e.target.checked })} /> Mostrar en Carrusel Web</label><label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', fontWeight: 600 }}><input type="checkbox" checked={promo.showTag} onChange={(e) => updatePromotion(promo.id, { showTag: e.target.checked })} /> Mostrar etiqueta "10% OFF"</label></div>
                </div>
              ))}
            </div>
          )}
